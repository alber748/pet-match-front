...
const ModalEditar: React.FC<Props> = ({ cerrarModal, datosIniciales, }) => {
    ...
    const handleFileChange = (nuevaImagen: File | null, url : string ) => {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.post("https://pet-match-backend.onrender.com/api/dogs/edit-perro", formDataToSend);
            console.log(response);
            cerrarModal();
        } catch (error) {
                    ...
                    <div className="modal-content px-3 pb-3">
                        ...
                        <form className="d-flex flex-column align-items-center form-publicacion gap-4 mt-4" onSubmit={handleSubmit}>
                            ...
                            <div className="d-flex justify-content-end gap-2">
                                ...
                                <a className='btn btn-danger' onClick={ deleteDog }>Elminar Registro</a>
                            </div>
                        </form>
                        </div>
                    </div>
